...
/*
 * mem_init - Initialize the memory system model using mmap to get contiguous pages from the memory
 */
void mem_init(void)
{
    mem_heap = (char *)mmap(
        NULL, /* kernel choose the address */
        MAX_HEAP,
        PROT_READ | PROT_WRITE,
        MAP_PRIVATE | MAP_ANONYMOUS,
        -1, /* no fd for anonymous mapping */
        0   // offset 0
    );
